...
for column in datetime_columns:
    ...
    orderpayment_ordercustomer_df[column] = pd.to_datetime(orderpayment_ordercustomer_df[column])
    orderitems_orders_df[column] = pd.to_datetime(orderitems_orders_df[column])
...
sum_order_items_df = create_sum_order_items(order_item_products_df)
bystate_df = create_bystate(customer_df)
rfm_df = create_rfm_df(orderpayment_ordercustomer_df)
sellers_df=create_seller_df(orderitems_orders_df)
#produol_price_df=create_productvol_price(order_item_products_df)ctv
...
#=======================================================Product volume vs Price =================================
st.subheader("Price vs Volume")

fig,ax=plt.subplots(figsize=(15,6))
sns.regplot(x='product_volume', y='price', data=order_item_products_df, marker='x', line_kws=dict(color="r"))
